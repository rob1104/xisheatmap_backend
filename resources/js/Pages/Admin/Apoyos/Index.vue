<template>
    <Head title="Historial de Apoyos" />

    <AdminLayout>
        <template #header>
            Historial de Apoyos Sociales
        </template>
        ...
        <div class="max-w-7xl mx-auto animate-fade-in-up">
            ...
            <div class="bg-slate-900 shadow-2xl rounded-2xl overflow-hidden border border-slate-800">
                <table class="min-w-full divide-y divide-slate-800">
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Apoyo</th>
                    ...
                    <tr v-for="apoyo in apoyos.data" :key="apoyo.id" class="hover:bg-slate-800/50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-indigo-400">{{ apoyo.nombre }}</div>
                            <div class="text-xs text-slate-500">{{ apoyo.telefono || 'Sin teléfono' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-slate-200">{{ apoyo.colonia }}</div>
                            <div class="text-xs text-slate-400 italic">{{ apoyo.calle_y_numero }}</div>
                            <div class="text-[10px] text-slate-600 font-mono mt-1">Lat: {{ apoyo.latitud }}, Lng: {{ apoyo.longitud }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                            {{ apoyo.apoyo }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span :class="apoyo.estatus_de_apoyo === 'Atendido' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'" class="px-3 py-1 rounded-md text-xs font-bold uppercase tracking-widest">
                                {{ apoyo.estatus_de_apoyo }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                            <button @click="openEditModal(apoyo)" class="text-slate-400 hover:text-indigo-400 transition-colors" title="Editar Apoyo">
                            ...
                            <button @click="deleteApoyo(apoyo.id)" class="text-slate-400 hover:text-red-400 transition-colors" title="Eliminar Apoyo">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </td>
                    </tr>
                    <tr v-if="apoyos.data.length === 0">
            ...
            <!-- Pagination -->
            <div class="mt-4 flex justify-between items-center" v-if="apoyos.links.length > 3">
                <div class="text-sm text-slate-400">
                    Mostrando {{ apoyos.from }} a {{ apoyos.to }} de {{ apoyos.total }} resultados
                </div>
                <div class="flex gap-1">
                    <template v-for="(link, p) in apoyos.links" :key="p">
        ...
        <dialog v-if="showModal" class="modal-open fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-sm p-4">
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                <h3 class="text-xl font-bold mb-6 text-slate-100">{{ isEditing ? 'Editar Apoyo' : 'Registrar Nuevo Apoyo' }}</h3>
                
                <form @submit.prevent="saveApoyo" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-400 mb-1">Apoyo</label>
                            <input v-model="form.apoyo" type="text" required class="w-full border-slate-700 bg-slate-950 text-slate-200 placeholder-slate-500 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-inner" placeholder="Ej. Lotería, Despensa...">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-400 mb-1">Estatus de Apoyo</label>
                            <select v-model="form.estatus_de_apoyo" required class="w-full border-slate-700 bg-slate-950 text-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-inner">
                    ...
                    <div class="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-800">
                        ...
                        <button type="submit" :disabled="form.processing" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 transition-colors text-white rounded-lg font-bold shadow-lg disabled:opacity-50">
                            {{ isEditing ? 'Guardar Cambios' : 'Crear Apoyo' }}
                        </button>
                    </div>
                </form>
            </div>
...
const doSearch = () => {
    router.get(route('apoyos.index'), { search: search.value }, { preserveState: true })
}

const openCreateModal = () => {
    isEditing.value = false
    selectedApoyo.value = null
    form.reset()
    showModal.value = true
}

const openEditModal = (apoyo) => {
    isEditing.value = true
    selectedApoyo.value = apoyo
    
    form.nombre = apoyo.nombre
    form.telefono = apoyo.telefono
    form.colonia = apoyo.colonia
    form.calle_y_numero = apoyo.calle_y_numero
    form.latitud = apoyo.latitud
    form.longitud = apoyo.longitud
    form.apoyo = apoyo.apoyo
    form.estatus_de_apoyo = apoyo.estatus_de_apoyo
...
const saveApoyo = () => {
    if (isEditing.value) {
        form.put(route('apoyos.update', selectedApoyo.value.id), {
            onSuccess: () => closeModal(),
            preserveScroll: true
        })
    } else {
        form.post(route('apoyos.store'), {
            onSuccess: () => closeModal(),
            preserveScroll: true
        })
    }
...
const deleteApoyo = (id) => {
    if(confirm('¿Estás seguro de eliminar este registro de apoyo?')) {
        router.delete(route('apoyos.destroy', id), {
            preserveScroll: true
        })
    }
}
